...
const HomePage = ()=>{
    ...
    const checkServerReady = async () => {
        try {
            const st = await axios.get(import.meta.env.VITE_SERVER_DOMAIN + "/status");
            // debugger;
            return true;
        } catch (err) {
            return false;
    ...
    useEffect(() => {
        ...
        if (!ready) {
            console.warn("Server not ready, retrying in 2s...");
            setTimeout(init, 1000); // try again after 2 seconds
            return;
        }

        // Once backend is ready → fetch initial blogs
        if (pageState === "home") {
